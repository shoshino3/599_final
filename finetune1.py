...
def train():
    ...
    for epoch in range(5):
        for i, batch in enumerate(dataloader):
            ...
            if i in range(10):
              print(f"Epoch: {epoch} | Iter: {i} | Loss: {loss.item() * iters_to_accumulate:.4f}")  # Adjust the displayed loss to reflect the                 actual loss per batch
    model_weight = model.state_dict()
    lora_wts = {k: v for k,v in model_weight.items() if 'lora_' in k}   
    torch.save(lora_wts, 'lora_weights.pth')     
